...
# Kernel functions now imported from kernel_utils module
...
class BAYES_MISP:
    def __init__(self, cfg, budgetSize, train_labels, lset, delta=1):
        ...
        self.update_K_matrix_factor = self.cfg.UPDATE_K_MATRIX_FACTOR if 'UPDATE_K_MATRIX_FACTOR' in self.cfg else 1.0
        self.update_C_matrix = self.cfg.UPDATE_C_MATRIX if 'UPDATE_C_MATRIX' in self.cfg else False
        ...
        sparsity_ratio, total_pairs, total_points = self.get_sparsity_ratio()
        ...
        print(f"Initial K sparsity ratio: {sparsity_ratio}")
    ...
    def get_sparsity_ratio(self):
        total_points = self.K_general.shape[0]
        ...
        return sparsity_ratio, total_pairs, total_points
    ...
    def build_K_general_matrix(self, features, threshold, zero_indices=None, prev_threshold=None):
        ...
        if self.use_sparse:
            ...
            capture_new_connections = (self.update_C_matrix and
            ...
            print("Built sparse K matrix in {:.2f} seconds.".format(time.time() - start_time))
            if capture_new_connections:
                ...
                if zero_contrib and zero_contrib["sources"].size > 0:
                    ...
                    print("Updated C with newly available connections from labeled set in {:.2f} seconds.".format(time.time() - start_time))
                return K_matrix
            return build_result
        ### unsparse K matrix
    ...
    def _extract_sparse_submatrix(self, K_csr, indices):
        """
        Efficiently extract a symmetric submatrix K[indices, :][:, indices] from a CSR matrix.
        
        Instead of double fancy indexing (which is slow for CSR column slicing),
        this converts to COO format, filters entries, and rebuilds the matrix.
        This avoids scipy's SparseEfficiencyWarning and is much faster.
        """
        indices = np.asarray(indices, dtype=np.int64)
        n_new = len(indices)
        
        # Create mapping from old indices to new indices
        # -1 means the index is not in our selection
        old_to_new = np.full(K_csr.shape[0], -1, dtype=np.int64)
        old_to_new[indices] = np.arange(n_new)
        ...
        row_new = old_to_new[row_old]
        col_new = old_to_new[col_old]
        
        # Keep only entries where both row and col are in our selection
        keep_mask = (row_new >= 0) & (col_new >= 0)
        
        if not keep_mask.any():
            return sp.csr_matrix((n_new, n_new), dtype=K_csr.dtype)
        
        # Build new COO matrix with filtered entries
        new_coo = sp.coo_matrix(
            (data[keep_mask], (row_new[keep_mask], col_new[keep_mask])),
            shape=(n_new, n_new),
            dtype=K_csr.dtype
        )
    ...
    def init_sampling_loop(self,lset, uset):
        ...
        self.activeSet = []
        if self.use_sparse:
            # Extract submatrix efficiently using COO intermediate format
            # This avoids the slow CSR column slicing which triggers SparseEfficiencyWarning
            K_csr_shuffled = self._extract_sparse_submatrix(self.K_general, self.relevant_indices)
    ...
    def select_samples(self, lset, uset):
        ...
        ## update general K
        if len(lset)>0 and len(lset) % 100 == 0 and self.use_sparse and self.update_K_matrix:
            self.update_K_general_matrix(lset)
            sparsity_ratio, total_pairs, total_points = self.get_sparsity_ratio()
            print("New K sparsity ratio after updating K: {:.6f}".format(sparsity_ratio))
        ...
        for i in range(self.budgetSize):
            ...
            if self.use_sparse:
                point_total_contribution = batched_diffs_efficient_weighted_sparse(self.K, self.C, cont_method=self.cont_method, weight_method=self.distribution_cont_weight_method, class_gains_weights=class_gains_weights)
            else:
                if len(self.K.shape) == 2:
                    self.K.unsqueeze_(2)
                point_total_contribution = batched_diffs_efficient_weighted(self.K, self.C,
                                                      diff_method="efficient_full_weighted_max",cont_method=self.cont_method, weight_method=self.distribution_cont_weight_method)
    ...
    def update_K_general_matrix(self, lset):
        ...
        new_threshold_euclidean_dist = sorted_values[::-1][self.initial_sparse_index - int(self.total_connections_chosen*self.update_K_matrix_factor)]
        print(f"Updating K general matrix with new threshold (euclidean dist): {new_threshold_euclidean_dist}")
        print(f"Total connections chosen so far: {self.total_connections_chosen}")
        if self.kernel_type == 'tophat':
